...
| Existing workflows | `ls .github/workflows/` | reconcile (Phase 2) if `immune.yml` exists; otherwise just note coexisting CI |
...
| Existing labels | `gh api repos/$OWNER_REPO/labels --jq '.[].name'` | reconcile (Phase 2) if `immune:*` present (color update if needed) |
...
| AGENTS.md mentions disclosure / quality-gate | `mode: gate` |
...
#### immune label vocabulary (terminal-only — three labels, all subtle gray)
...
Action source: [kimjune01/immune](https://github.com/kimjune01/immune) (AGPL-3.0; pinned to a specific tag in the workflow file). The action runs in your runner with your secrets; nothing is sent to a kimjune01-controlled endpoint.
```
...
The two-PR pair is the load-bearing design choice: an install that only opens a STRONG PR can hide a too-lenient attend, and an install that only opens a WEAK PR can hide a too-strict filter. Both pin the calibration from both sides. The model split (opus + manual receipts for STRONG, haiku for WEAK) makes the weakness *organic* — what haiku produces when not asked for receipts is what unattended AI-generated PRs typically look like, which is exactly the calibration target. Per [[feedback-attestation-universal]], every change needs a receipt — for an install, the receipt is the verdict pair.
